...
app.get ('/strava-subscriptions', function (request, response,err){
//response.json({hub.challenge: request.query['hub.challenge']});
console.log (request.query["hub.challenge"]);
response.json ({["hub.challenge"]:request.query["hub.challenge"]});
});

// Webhook POST destination from Strava
app.post ('/strava-subscriptions', function (request, response,err){
emitActivityEvent()
